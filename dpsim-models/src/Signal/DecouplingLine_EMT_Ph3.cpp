...
#include <dpsim-models/Signal/DecouplingLine_EMT_Ph3.h>

using namespace CPS;
using namespace CPS::EMT::Ph3;
using namespace CPS::Signal;

DecouplingLine_EMT_Ph3::DecouplingLine_EMT_Ph3(String name, Logger::Level logLevel)
...
void DecouplingLine_EMT_Ph3::setParameters(SimNode<Real>::Ptr node1,
...
void DecouplingLine_EMT_Ph3::initialize(Real omega, Real timeStep) {
  if (mDelay < timeStep)
...
Matrix DecouplingLine_EMT_Ph3::interpolate(Matrix &data) {
...
void DecouplingLine_EMT_Ph3::step(Real time, Int timeStepCount) {
...
void DecouplingLine_EMT_Ph3::PreStep::execute(Real time, Int timeStepCount) {
  mLine.step(time, timeStepCount);
}

void DecouplingLine_EMT_Ph3::postStep() {
...
void DecouplingLine_EMT_Ph3::PostStep::execute(Real time, Int timeStepCount) {
  mLine.postStep();
}

Task::List DecouplingLine_EMT_Ph3::getTasks() {
  return Task::List(
      {std::make_shared<PreStep>(*this), std::make_shared<PostStep>(*this)});
}

IdentifiedObject::List DecouplingLine_EMT_Ph3::getLineComponents() {
