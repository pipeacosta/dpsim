...
int main(int argc, char *argv[]) {
  ...
  // if (argc <= 1) {
    // filenames = Utils::findFiles({"SWW_Wunsiedel_CIM_export_EQ.xml",
    //                               "SWW_Wunsiedel_CIM_export_SV.xml", "SWW_Wunsiedel_CIM_export_TP.xml"},
    //                              "build/_deps/cim-data-src/SWW_Wunsiedel_CIM_export/",
    //                              "CIMPATH");

    if (argc <= 1) {
    filenames = Utils::findFiles({"21060207T0628Z_XX_YYY_EQ.xml",
                                  "21060207T0628Z_XX_YYY_SV.xml", "21060207T0628Z_XX_YYY_TP.xml"},
                                 "build/_deps/cim-data-src/21060207T0628Z_XX_YYY/",
                                 "CIMPATH");
    timeStep = 100e-6;
    finalTime = 0.1;
  } else {
  ...
  SystemTopology systemPF = reader.loadCIM(
      50, filenames, Domain::SP, PhaseType::Single, CPS::GeneratorType::PVNode);
  ...
  SystemTopology sys =
      reader2.loadCIM(50, filenames, Domain::DP, PhaseType::Single,
